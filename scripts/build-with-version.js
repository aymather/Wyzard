...
// Bundle Poppler dependencies first (macOS only)
if (process.platform === "darwin") {
  console.log("Step 1: Bundling Poppler dependencies...");
  try {
    execSync("yarn bundle-poppler-deps", {
      stdio: "inherit",
      cwd: path.join(__dirname, ".."),
    });
    console.log("✅ Poppler dependencies bundled\n");
  } catch (error) {
    console.error("❌ Failed to bundle Poppler dependencies:", error.message);
    console.error(
      "⚠️  Continuing anyway, but the app may not work without system dependencies"
    );
  }
}
...
console.log(
  `Step ${process.platform === "darwin" ? "2" : "1"}: Building React app...`
);
try {
  execSync("yarn build", {
    stdio: "inherit",
...
console.log(
  `Step ${process.platform === "darwin" ? "3" : "2"}: Packaging Electron app...`
);
try {
  ...
  if (fs.existsSync(path.join(outputDir, "mac"))) {
    const macFiles = fs
      .readdirSync(path.join(outputDir, "mac"))
  ...
  const winFiles = fs.readdirSync(outputDir).filter((f) => f.endsWith(".exe"));
